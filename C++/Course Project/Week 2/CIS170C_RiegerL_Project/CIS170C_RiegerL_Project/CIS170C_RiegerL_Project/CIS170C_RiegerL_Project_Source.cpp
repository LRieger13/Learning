...
void mainMenu() {
	string userAnswer;
cout << "What lesson would you like to start? A) Variable Declaration B) Input / Output C) Pop Quiz! OR EXIT\n";
cout << "Key in A, B, C, or EXIT for your answer: " << endl;
std::cin >> userAnswer;
...
void levelUp(string correctAnswer, string userVariable) {
	...
	if (correctAnswer == userVariable) {
		cout << "WooHoo! Ready for the next one?\n"; // if no & return need to call menu funtion
		std::cin >> nextLevel;
	}
}
...
void popQuiz() {
	...
	const string testQuestions[5] = { "Which Line Contains the Variable Declarations?", "Which line is the first place collecting input from the user?", "Which line is the last output?", "How would you get input?", "How would you print out the user's name?" };
	const string correctTestAnswers[5] = { "1", "5", "6", "cin >>", "cout << userName;" };
