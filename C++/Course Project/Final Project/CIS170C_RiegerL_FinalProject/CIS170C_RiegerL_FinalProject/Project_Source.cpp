...
MyClass::MyClass() {
	...
	string userName;
	cout << "WOOPS! Almost forgot, what is your name newbie? \n";
	std::cin >> userName;
	setUserName(userName);
	cout << "Nice to meet you " << userName << ". Let's get started with the basics!\n";
...
void MyClass::popQuiz() {
	string FileName = "TestAnswers.txt";

	char userChoice = ' ';
	//variables
	int i, numberCorrect = 0;
	const int SIZE = 6;
	string userTestAnswers[SIZE];
	const string testQuestions[] = { "Which Line Contains the Variable Declarations?", "Which line is the first place collecting input from the user?", "Which line is the last output?", "Which is the command for get input?", "Which kind of loop executes a line of code before checking the condition?", "How would you write an empty array for 3 names?" };
	const string correctTestAnswers[] = { "1", "5", "6", "cin", "do While", "names[3] = {};" };
...
|                        `.    `T-            |
 `----------------------._ \    |             |
	...
	//Add ability to save to disk
	do {
		//ask for user's choice
		std::cout << "Would you like to A) Save Answers, B) Show Past Answers, or E) Exit\n";
		std::cin >> userChoice;

		userChoice = toupper(userChoice);

		if (userChoice == 'A') {
			writeData(userTestAnswers, FileName);
		}
		if (userChoice == 'B') {
			readData(FileName);
		}
...
void MyClass::writeData(string arrayOfStringsToSave[6], string FileName) {

	ofstream outMyStream(FileName, ios::in);

	for (int i = 0; i < 6; i++) {
		//'#' is the delimiter
		outMyStream << arrayOfStringsToSave[i] << "#";
	}
...
void MyClass::readData(string FileName) {
	...
	if (inMyStream.is_open()) {
		...
		int fieldCount = 0; // num fields read
		int recordCount = 1; // num records read
